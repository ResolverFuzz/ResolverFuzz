...
is_powerdns_eval = False
is_knot_eval = False
is_maradns_eval = False
is_technitium_eval = False
...
result_folder_path = '/mnt/ssd1/dns_fuzzing_docker/fuzzer_results/fwd-fallback_test'
conf_folder_path = '/home/qifanz/DSP_Lab/dns-fuzzing/dns-docker-images/conf_fwd_fallback'
...
dump_folder_path = '/mnt/ssd1/dns_fuzzing_docker/fwd_fallback_dump'
...
class TestUnit:
	...
	def save_auth_payload(self, curr_response, json_data=None):
		if len(curr_response) != 2:
			return False
		auth_response = curr_response[0].hex() + '\t' \
			+ curr_response[1].hex() + '\n'
	...
	def bind9_pipeline(self, curr_query):
		...
		print('curr bind9 response:', curr_bind9_response.hex())
	...
	def unbound_pipeline(self, curr_query):
		...
		print('curr unbound response:', curr_unbound_response.hex())
	...
	def powerdns_pipeline(self, curr_query):
		...
		print('curr powerdns response:', curr_powerdns_response.hex())
	...
	def knot_pipeline(self, curr_query):
		...
		print('curr knot response:', curr_knot_response.hex())
	...
	def maradns_pipeline(self, curr_query):
		...
		print('curr maradns response:', curr_maradns_response.hex())
	...
	def technitium_pipeline(self, curr_query):
		...
		print('curr technitium response:', curr_technitium_response.hex())
...
class TechnitiumContainer:
	def __init__(self,
		...
		# forwarder domain and ip addr
		self.forward_domain = 'test{}.qifanzhang.com'.format(self.unit_num)
		self.forward_ipv4_addr = '{prefix}.{unit_num}'.format(
	...
	def set_cdns(self, user="admin", password="admin"):
		token = self.get_token(user, password)

		payload = {'token': token, 'zone': self.forward_domain, 'type': 'Forwarder', 'forwarder': self.forward_ipv4_addr, 'dnssecValidation': 'false'}
		r = requests.get('http://' + self.ipv4_addr + ':5380/api/zones/create', params=payload)
		print('technitium set_cdns request:', r.url)
		if r.status_code == 200 and r.json()['status'] == 'ok':
			return r.json()
		return
...
class AuthSrvContainer:
	...
	def start(self, response_payload):
		exit_code, output = self.container.exec_run('python3 /auth-srv-tmp/dns_auth_srv.py {response0} {response1} {auth_srv_ip}'.format(
				response0 = response_payload[0].hex(),
				response1 = response_payload[1].hex(), 
...
class DNSFuzzerGenerator:
	def __init__(self, unit_num):
		self.domain_self = 'test{}.qifanzhang.com'.format(unit_num)
