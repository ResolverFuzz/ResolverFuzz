...
1  * 0.123 = 0.122 (ref. single-thread throughput)
5  * 0.121 = 0.605
10 * 0.117 = 1.172
15 * 0.116 = 1.733
20 * 0.111 = 2.221
...
result_folder_path = '/mnt/ssd1/dns_fuzzing_docker/fuzzer_results/fwd-global_test'
conf_folder_path = '/home/qifanz/DSP_Lab/dns-fuzzing/dns-docker-images/conf_fwd_global'
...
dump_folder_path = '/mnt/ssd1/dns_fuzzing_docker/fwd_global_dump'
...
class TestUnit:
	...
	def technitium_pipeline(self, curr_query):
		...
		if technitium_is_crashed:
			self.technitium_obj.restart(curr_unit_num=self.unit_num)
...
class Bind9Container:
	...
	def create(self, curr_unit_num:int):
		print('conf folder path:', '{conf_folder_path}/bind9/{unit_num}'.format(
		...
		self.container = client.containers.run(self.image_name,
			name = self.container_name, 
			volumes = [
				'{conf_folder_path}/bind9/{unit_num}:/etc/bind'.format(
					conf_folder_path = conf_folder_path, 
					unit_num = curr_unit_num
				), 
...
class UnboundContainer:
	...
	def create(self, curr_unit_num:int):
		print('conf folder path:', '{conf_folder_path}/unbound/{unit_num}'.format(
			conf_folder_path = conf_folder_path, 
			unit_num = curr_unit_num
		))
		print('dump folder path:', self.dump_folder)
		...
		self.container = client.containers.run(self.image_name,
			name = self.container_name, 
			volumes = [
				'{conf_folder_path}/unbound/{unit_num}:/etc/unbound'.format(
					conf_folder_path = conf_folder_path, 
					unit_num = curr_unit_num
				), 
...
class PowerDNSContainer:
	...
	def create(self, curr_unit_num:int):
		print('conf folder path:', '{conf_folder_path}/powerdns/{unit_num}'.format(
			conf_folder_path = conf_folder_path, 
			unit_num = curr_unit_num
		))
		print('dump folder path:', self.dump_folder)
		...
		self.container = client.containers.run(self.image_name,
			name = self.container_name, 
			volumes = [
				'{conf_folder_path}/powerdns/{unit_num}:/etc/powerdns'.format(
					conf_folder_path = conf_folder_path, 
					unit_num = curr_unit_num
				), 
...
class KnotContainer:
	...
	def create(self, curr_unit_num:int):
		print('conf folder path:', '{conf_folder_path}/knot/{unit_num}'.format(
			conf_folder_path = conf_folder_path, 
			unit_num = curr_unit_num
		))
		print('dump folder path:', self.dump_folder)
		...
		self.container = client.containers.run(self.image_name,
			name = self.container_name, 
			volumes = [
				'{conf_folder_path}/knot/{unit_num}:/etc/knot-resolver'.format(
					conf_folder_path = conf_folder_path, 
					unit_num = curr_unit_num
				), 
...
class MaraDNSContainer:
	...
	def create(self, curr_unit_num:int):
		print('conf folder path:', '{conf_folder_path}/maradns/{unit_num}'.format(
			conf_folder_path = conf_folder_path, 
			unit_num = curr_unit_num
		))
		print('dump folder path:', self.dump_folder)
		...
		self.container = client.containers.run(self.image_name,
			name = self.container_name, 
			volumes = [
				'{conf_folder_path}/maradns/{unit_num}:/etc/maradns_conf'.format(
					conf_folder_path = conf_folder_path, 
					unit_num = curr_unit_num
				), 
...
class TechnitiumContainer:
	...
	def start(self, curr_unit_num:int):
		...
		self.set_config()
		self.set_fwd_global(curr_unit_num=curr_unit_num)
	...
	def set_fwd_global(self, curr_unit_num:int, user="admin", password="admin"):
		token = self.get_token(user, password)

		payload = {
			'token': token, 
			'forwarders': '{technitium_ipv4_prefix}.{unit_num}'.format(
				technitium_ipv4_prefix = technitium_ip_addr_prefix, 
				unit_num = curr_unit_num
			), 
			'forwarderProtocol': 'Udp'
		}
		r = requests.get('http://' + self.ipv4_addr + ':5380/api/settings/set', params=payload)
		print('technitium set_fwd_global request:', r.url)
		if r.status_code == 200 and r.json()['status'] == 'ok':
			return r.json()
		return None
...
if __name__ == '__main__':
	...
	for curr_unit_num in range(unit_size):
		...
		if is_bind9_eval:
			...
			curr_bind9_obj.create(curr_unit_num=curr_unit_num)
		...
		if is_unbound_eval:
			...
			curr_unbound_obj.create(curr_unit_num=curr_unit_num)
		...
		if is_powerdns_eval:
			...
			curr_powerdns_obj.create(curr_unit_num=curr_unit_num)
		...
		if is_knot_eval:
			...
			curr_knot_obj.create(curr_unit_num=curr_unit_num)
		...
		if is_maradns_eval:
			...
			curr_maradns_obj.create(curr_unit_num=curr_unit_num)
		...
		if is_technitium_eval:
			...
			curr_technitium_obj.start(curr_unit_num=curr_unit_num)
